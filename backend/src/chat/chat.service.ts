...
Средние траты: ${(totalExpenses / 30).toLocaleString('ru-RU')} ₽/день`;
  ...
  private generateBudgetAnalysis(user: any): string {
    if (!user?.budgets?.length) {
      return 'У вас пока нет бюджетов. Создайте бюджеты для категорий, чтобы отслеживать расходы.';
    }

    const analysis = user.budgets.map((b: any) => {
      return `${b.category?.name || 'Категория'}: ${b.amount}₽/мес`;
    }).join('\n');

    return `📋 **Анализ бюджетов**

${analysis}

Совет: Придерживайтесь лимитов в начале месяца, чтобы оставались средства на важные покупки к концу.`;
  }
