    ...
    return (
      <Pressable onPress={() => openEdit(item)} style={{ backgroundColor: CARD_BG, borderRadius: 16, borderWidth: 1, borderColor: BORDER, padding: 18, marginBottom: 10 }}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          ...
          <Text style={{ fontSize: 12, color: '#52525B' }}>{t("common.of")} {formatCurrency(item.amount)}</Text>
        </View>
      </Pressable>
    );
  }, [categories, openEdit]);
    ...
    <View style={{ flex: 1, backgroundColor: '#0A0A0F', paddingTop: insets.top }}>
      <View style={{ position: 'relative' }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 16 }}>
          ...
          <Text style={{ fontSize: 22, fontWeight: '700', color: '#FFFFFF', letterSpacing: -0.3 }}>{t("budget.title")}</Text>
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        <View style={{ backgroundColor: CARD_BG, borderRadius: 16, borderWidth: 1, borderColor: BORDER, padding: 18, marginBottom: 10 }}>
          ...
          <Text style={{ fontSize: 13, color: '#71717A', fontWeight: '500', marginBottom: 6 }}>{t("budget.freeThisMonth")}</Text>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 12, borderTopWidth: 1, borderTopColor: BORDER }}>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 12, color: '#52525B', marginBottom: 3 }}>{t("budget.monthlyLimitLabel")}</Text>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 12, color: '#52525B', marginBottom: 3 }}>{t("budget.spent")}</Text>
              <Text style={{ fontSize: 15, fontWeight: '600', color: '#F87171' }}>{formatCurrency(totalSpent)}</Text>
            </View>
          </View>
        </View>

        <Text style={{ fontSize: 16, fontWeight: '600', color: '#A1A1AA', marginBottom: 10, marginTop: 4 }}>{t("budget.budgetByCategory")}</Text>

        {budgets.length === 0 ? (
          <View style={{ alignItems: 'center', paddingVertical: 40 }}>
            <Ionicons name="pie-chart-outline" size={44} color="#3F3F46" />
            <Text style={{ fontSize: 14, color: '#3F3F46', marginTop: 8 }}>{t("budget.noBudgets")}</Text>
          </View>
        ) : (
          <FlatList
            data={budgets}
        ...
        <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} onPress={() => setShowAdd(false)}>
          <Pressable style={{ backgroundColor: '#1C1C20', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 16, paddingBottom: 32 }} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <Text style={{ fontSize: 18, fontWeight: '700', color: '#FFFFFF' }}>{t("budget.newBudget")}</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 500 }}>
              <View style={{ gap: 16 }}>
                <Text style={{ fontSize: 13, color: '#71717A', fontWeight: '500', marginBottom: 6 }}>{t("budget.categoryLabel")}</Text>
                ...
                <Text style={{ fontSize: 13, color: '#71717A', fontWeight: '500', marginBottom: 6 }}>{t("budget.monthlyLimitLabel")}</Text>
                <TextInput
                ...
                <Text style={{ fontSize: 13, color: '#71717A', fontWeight: '500', marginBottom: 6 }}>{t("budget.warningThreshold", { threshold: alertThreshold })}</Text>
                ...
                >
                  <Text style={{ fontSize: 14, fontWeight: '700', color: '#FFFFFF' }}>{t("common.create")}</Text>
                </Pressable>
              </View>
            </ScrollView>
          </Pressable>
      ...
      <Modal visible={editModal.visible} transparent animationType="slide" onRequestClose={() => setEditModal({ visible: false, budget: null, amount: '', alertThreshold: 80 })}>
        <Pressable style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }} onPress={() => setEditModal({ visible: false, budget: null, amount: '', alertThreshold: 80 })}>
          <Pressable style={{ backgroundColor: '#1C1C20', borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingHorizontal: 20, paddingTop: 16, paddingBottom: 32 }} onPress={(e) => e.stopPropagation()}>
            <View style={{ width: 36, height: 4, borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.15)', alignSelf: 'center', marginBottom: 16 }} />
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <Text style={{ fontSize: 18, fontWeight: '700', color: '#FFFFFF' }}>{t("budget.editBudget")}</Text>
            ...
            <View style={{ gap: 16 }}>
              ...
              <Text style={{ fontSize: 13, color: '#71717A', fontWeight: '500', marginBottom: 6 }}>{t("budget.warningThreshold", { threshold: editModal.alertThreshold })}</Text>
              <View style={{ flexDirection: 'row', gap: 8 }}>
                {[50, 70, 80, 90].map((t) => (
                  <Pressable
                    key={t}
              ...
              >
                <Text style={{ fontSize: 14, fontWeight: '600', color: '#F87171' }}>{t("budget.deleteBudget")}</Text>
              </Pressable>
            </View>
          </Pressable>
        </Pressable>
